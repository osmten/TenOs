...
void* temp_map_page(u32 phys_addr) {
    // Map the physical page at TEMP_MAP_ADDR (a reserved kernel virtual address)
    // vmm_map_page_late(kernel_directory, (void*)TEMP_MAP_ADDR, (void*)phys_addr,
    //                 I86_PTE_PRESENT | I86_PTE_WRITABLE | I86_PTE_USER);


    vmmngr_map_page((void*)phys_addr, (void*)TEMP_MAP_ADDR);
    
    // Flush TLB for this address so CPU sees the new mapping
    __asm__ volatile("invlpg (%0)" :: "r"(TEMP_MAP_ADDR) : "memory");
...
void process_init() {
    pr_info("PROC", "Initializing process system.....\n");
    memset(process_list, 0, sizeof(process_list));
    pr_info("PROC", "Process subsystem initialization done...\n");
}
...
/* ============================================================================
 * PROCESS CREATION - FIXED VERSION
 * ============================================================================ */
...
struct process *process_create(const char *name, void *code, int code_size) {
     ...
     * FIXED: Convert physical address to virtual address
     * ------------------------------------------------------------------------ */
    void *proc_phys = alloc_memory_block();
    if (!proc_phys) {
        pr_err("PROC", "Failed to allocate process structure\n");
        return NULL;
    }
    
    // CRITICAL: Convert physical to virtual address!
    struct process *proc = (struct process *)P2V((u32)proc_phys);
    memset(proc, 0, sizeof(struct process));
    
    printk("  Process struct: phys=0x%x, virt=0x%x\n", 
           (u32)proc_phys, (u32)proc);
    
    /* ------------------------------------------------------------------------
     * STEP 2: Initialize basic process information
     * ------------------------------------------------------------------------ */
    proc->pid = next_pid++;
    int size_name = strlen(name);
    if (size_name > sizeof(proc->name) - 1) {
        size_name = sizeof(proc->name) - 1;
    }
    memcpy(proc->name, name, size_name);
    proc->name[size_name] = '\0';
    proc->state = PROCESS_READY;
    ...
    /* ------------------------------------------------------------------------
     * STEP 3: Allocate kernel stack (2 pages = 8KB)
     * FIXED: alloc_memory_blocks returns physical address
     * ------------------------------------------------------------------------ */
    void *kernel_stack_phys = alloc_memory_blocks(2);
    if (!kernel_stack_phys) {
        pr_err("PROC", "Failed to allocate kernel stack\n");
        free_memory_block(proc_phys);
        return NULL;
    }
    
    // Store as VIRTUAL address for easy access
    proc->kernel_stack = (u32)P2V((u32)kernel_stack_phys);
    
    printk("  Kernel stack: phys=0x%x, virt=0x%x - 0x%x\n", 
           (u32)kernel_stack_phys, 
           proc->kernel_stack, 
           proc->kernel_stack + KERNEL_STACK_SIZE);
    
    /* ------------------------------------------------------------------------
     * STEP 4: Allocate and initialize page directory
     * FIXED: Store physical address, use temp mapping to access
     * ------------------------------------------------------------------------ */
    void *pd_phys = alloc_memory_block();
    if (!pd_phys) {
        pr_err("PROC", "Failed to allocate page directory\n");
        free_memory_blocks(kernel_stack_phys, 2);
        free_memory_block(proc_phys);
        return NULL;
    }
    
    // Store PHYSICAL address (needed for CR3)
    proc->page_dir_phys = (u32)pd_phys;
    
    printk("  Page directory: phys=0x%x\n", (u32)pd_phys);
    
    /* ------------------------------------------------------------------------
     * STEP 5: Initialize page directory via temporary mapping
     * FIXED: Use temp_map_page to access physical page directory
     * ------------------------------------------------------------------------ */
    struct pdirectory *proc_pd = (struct pdirectory*)temp_map_page((u32)pd_phys);
    memset(proc_pd, 0, PAGE_SIZE);
    
    // Copy kernel mappings (entries 768-1023 = 3GB-4GB)
    for (int i = 768; i < 1024; i++) {
        proc_pd->m_entries[i] = kernel_directory->m_entries[i];
    }
    ...
    /* ------------------------------------------------------------------------
     * STEP 6: Calculate memory regions
     * ------------------------------------------------------------------------ */
    u32 num_code_pages = (code_size + PAGE_SIZE - 1) / PAGE_SIZE;
    ...
    // User stack at 2GB boundary
    proc->stack_start = 0x80000000;
    proc->stack_end = 0x7FFFF000;
    
    printk("  Code:  0x%x - 0x%x (%u pages)\n", 
           proc->code_start, proc->code_end, num_code_pages);
    printk("  Stack: 0x%x - 0x%x\n", 
           proc->stack_end, proc->stack_start);
    
    /* ------------------------------------------------------------------------
     * STEP 7: Map code pages
     * FIXED: All physical addresses from alloc_memory_block
     * ------------------------------------------------------------------------ */
    for (u32 i = 0; i < num_code_pages; i++) {
        // Allocate physical frame for this code page
        void *phys = alloc_memory_block();
        if (!phys) {
            pr_err("PROC", "Failed to allocate code page %u\n", i);
            temp_unmap_page(proc_pd);
            // TODO: Proper cleanup of all allocated resources
            return NULL;
        }
        ...
        // Check if page table exists for this address range
        if (!(proc_pd->m_entries[pd_idx] & I86_PDE_PRESENT)) {
            // Page table doesn't exist, create it
            void *pt_phys = alloc_memory_block();
            if (!pt_phys) {
                pr_err("PROC", "Failed to allocate page table\n");
                temp_unmap_page(proc_pd);
                return NULL;
            }
            
            // CRITICAL: Access page table via P2V
            struct ptable *pt = (struct ptable*)P2V((u32)pt_phys);
            memset(pt, 0, sizeof(struct ptable));
            
            // Add page table to page directory (store PHYSICAL address)
            proc_pd->m_entries[pd_idx] = (u32)pt_phys | I86_PDE_PRESENT | 
                                         I86_PDE_WRITABLE | I86_PDE_USER;
        }
        
        // Get the page table (extract physical, convert to virtual)
        u32 pt_phys = proc_pd->m_entries[pd_idx] & ~0xFFF;
        struct ptable *pt = (struct ptable*)P2V(pt_phys);
        
        // Add mapping (store PHYSICAL frame address)
        pt->m_entries[pt_idx] = (u32)phys | I86_PTE_PRESENT | 
                                I86_PTE_WRITABLE | I86_PTE_USER;
    }
    
    printk("  Code pages mapped\n");
    
    /* ------------------------------------------------------------------------
     * STEP 8: Map user stack
     * ------------------------------------------------------------------------ */
    void *stack_phys = alloc_memory_block();
    if (!stack_phys) {
        pr_err("PROC", "Failed to allocate stack page\n");
        temp_unmap_page(proc_pd);
        return NULL;
    }
    ...
    if (!(proc_pd->m_entries[pd_idx] & I86_PDE_PRESENT)) {
        void *pt_phys = alloc_memory_block();
        if (!pt_phys) {
            pr_err("PROC", "Failed to allocate page table for stack\n");
            temp_unmap_page(proc_pd);
            return NULL;
        }
    ...
    u32 pt_phys = proc_pd->m_entries[pd_idx] & ~0xFFF;
    struct ptable *pt = (struct ptable*)P2V(pt_phys);
    pt->m_entries[pt_idx] = (u32)stack_phys | I86_PTE_PRESENT | 
                            I86_PTE_WRITABLE | I86_PTE_USER;
    
    printk("  Stack mapped\n");
    
    // Done with temporary page directory mapping
    temp_unmap_page(proc_pd);
    
    /* ------------------------------------------------------------------------
     * STEP 9: Copy code and initialize stack
     * ------------------------------------------------------------------------ */
    u32 old_cr3;
    __asm__ volatile("mov %%cr3, %0" : "=r"(old_cr3));
    
    printk("  Switching to process PD (0x%x -> 0x%x)\n", old_cr3, proc->page_dir_phys);
    __asm__ volatile(
        "mov %0, %%cr3\n\t"
        "jmp 1f\n\t"
        "1:\n\t"
        "nop"
        :: "r"(proc->page_dir_phys)
        : "memory"
    );
    
    // Copy code into process's address space
    for (u32 i = 0; i < num_code_pages; i++) {
        u32 virt = proc->code_start + (i * PAGE_SIZE);
        
        int copy_size = PAGE_SIZE;
        if (i == num_code_pages - 1 && code_size % PAGE_SIZE) {
            copy_size = code_size % PAGE_SIZE;
        }
    ...
    printk("  Code copied and stack initialized\n");
    ...
    printk("  Switched back to kernel PD\n");
    
    /* ------------------------------------------------------------------------
     * STEP 10: Initialize CPU registers
     * ------------------------------------------------------------------------ */
    memset(&proc->regs, 0, sizeof(proc_registers_t));
    ...
    /* ------------------------------------------------------------------------
     * STEP 11: Add to process list
     * ------------------------------------------------------------------------ */
    if (add_process(proc) < 0) {
        pr_err("PROC", "Failed to add process to list\n");
        return NULL;
    }
...
void process_run(struct process *proc) {
    ...
    printk("  ESP:    0x%x\n", proc->regs.esp);
    printk("  EFLAGS: 0x%x\n", proc->regs.eflags);
    printk("  CS:     0x%x (Ring %d)\n", proc->regs.cs, proc->regs.cs & 0x3);
    printk("  SS:     0x%x (Ring %d)\n", proc->regs.ss, proc->regs.ss & 0x3);
    
    // Verify TSS is loaded
    u16 tr;
    __asm__ volatile("str %0" : "=r"(tr));
    printk("\nTask Register: 0x%x ", tr);
    
    if (tr == 0) {
        pr_err("PROC", "TSS not loaded! Call tss_init() first.\n");
        return;
    }
    printk("(OK)\n");
