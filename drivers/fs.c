    ...
    // // extern void write_sector(uint32_t lba, const uint8_t *buffer);

    // static FAT12_MountInfo mount_info;
    // static uint8_t sector_buffer[1];
    ...
    // //     // mount_info.fat_buffer = vmmngr_alloc_page(mount_info.sectors_per_fat * 512);
    // //     mount_info.fat_buffer = vmmngr_alloc_page();
    ...
    // //     // Load entire FAT
    // //     for (uint32_t i = 0; i < mount_info.sectors_per_fat; i++) {
    ...
    // //     for (uint32_t sector = 0; sector < mount_info.root_size; sector++) {
    ...
    // //             if(0) {
    // //                 // Found it!
    // //                 // FAT12_File *file = vmmngr_alloc_page(sizeof(FAT12_File));
    // //                 FAT12_File *file = vmmngr_alloc_page();
    ...
    // //     // Read 16-bit value
    // //     uint16_t next_cluster = *(uint16_t*)(mount_info.fat_buffer + fat_offset);
    ...
    // // static void set_cluster_value(uint16_t cluster, uint16_t value) {
    // //     uint32_t fat_offset = cluster + (cluster / 2);
    // //     uint16_t *fat_entry = (uint16_t*)(mount_info.fat_buffer + fat_offset);
    ...
    // //         uint32_t lba = mount_info.data_start + 
    ...
    // //     for (uint16_t cluster = 2; cluster < 4084; cluster++) {
    // //         uint16_t value = get_next_cluster(cluster);
    // //         if (value == FAT12_FREE) {
    // //             return cluster;
    // //         }
    // //     }
    ...
    // // static DirectoryEntry* find_free_dir_entry(uint32_t *sector_out, uint32_t *index_out) {
    // //     for (uint32_t sector = 0; sector < mount_info.root_size; sector++) {
    // //         read_sector(mount_info.root_start + sector, sector_buffer);
    // //         DirectoryEntry *entries = (DirectoryEntry*)sector_buffer;
    ...
    // //     uint32_t clusters_needed = (size + 511) / 512;  // Round up
    ...
    // //         // Copy data
    // //         uint32_t bytes_to_write = (size > 512) ? 512 : size;
